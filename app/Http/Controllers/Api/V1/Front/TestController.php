...
class TestController extends Controller
{
    public function index(TestRequest $request)
    {
        $tests = Test::where('user_id', $request->user_id)
            ->when($request->quiz_id, function ($query) use ($request) {
                ...
                ->with('quiz')
                ->select('quiz_id', 'user_id', 'started_at', 'finished_at', 'time_spent', 'questions_count', 'correct_questions_count', 'percent')
    ...
    public function update(UpdateTestRequest $request, Test $test)
    {
        ...
        if ($questionIds !== $dataQuestionIds) {
            throw ValidationException::withMessages([
                'questions' => 'Please, send all questions correctly.'
            ]);
        }
